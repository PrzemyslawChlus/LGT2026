...
import { Player, Match, LeagueSettings, StoredUser, SystemLog, LeagueNotification } from '../types';
import { INITIAL_PLAYERS, INITIAL_MATCHES, INITIAL_SETTINGS } from '../data/initialData';
import { INITIAL_USERS, COMMISSIONER_EMAIL } from '../utils/auth';
...
export async function cleanDatabaseForNewSeason(): Promise<void> {
  try {
    console.log('🧹 Purging old player database and initializing clean new season for Commissioner Przemysław Chłuś...');
    ...
    // 2. Delete all players except p1 and set p1 to Przemysław Chłuś
    const playersSnap = await getDocs(collection(db, PLAYERS_COLLECTION));
    const playerBatch = writeBatch(db);
    playersSnap.docs.forEach((d) => {
      if (d.id !== 'p1') {
        playerBatch.delete(d.ref);
      }
    });
    playerBatch.set(doc(db, PLAYERS_COLLECTION, 'p1'), INITIAL_PLAYERS[0]);
    await playerBatch.commit();

    // 3. Delete all users except u_admin and set u_admin to Commissioner
    ...
    userBatch.set(doc(db, USERS_COLLECTION, 'u_admin'), INITIAL_USERS[0]);
    await userBatch.commit();

    // 4. Update settings
    await setDoc(doc(db, SETTINGS_COLLECTION, SETTINGS_DOC_ID), INITIAL_SETTINGS);
    console.log('✅ Database is clean and ready for new season with Commissioner przemyslaw.chlus@gmail.com.');
  } catch (err) {
...
/**
 * Initializes and seeds Firestore with initial league data if collections are empty or contain legacy mock data.
 */
export async function ensureInitialSeed(): Promise<void> {
  if (isSeeding) return;
  try {
    // Check if legacy mock player p2 or old commissioner email exists in Firestore
    const legacyP2 = await getDoc(doc(db, PLAYERS_COLLECTION, 'p2'));
    const legacyAdmin = await getDoc(doc(db, USERS_COLLECTION, 'u_admin'));
    if (legacyP2.exists() || (legacyAdmin.exists() && legacyAdmin.data()?.email !== COMMISSIONER_EMAIL)) {
      isSeeding = true;
    ...
    if (playersSnapshot.empty) {
      isSeeding = true;
      console.log('🌱 Seeding initial league data into Firestore...');
      const batch = writeBatch(db);

      // Seed players (only Commissioner)
      for (const player of INITIAL_PLAYERS) {
        batch.set(doc(db, PLAYERS_COLLECTION, player.id), player);
      }

      // Seed matches (empty for new season)
      ...
      // Seed initial users (Commissioner)
      for (const user of INITIAL_USERS) {
        batch.set(doc(db, USERS_COLLECTION, user.id), user);
      }

      await batch.commit();
      console.log('✅ Initial league data successfully seeded to Firestore.');
    }
...
/**
 * Real-time subscription to user accounts list.
 */
export function subscribeToUsers(onUpdate: (users: StoredUser[]) => void): Unsubscribe {
  const colRef = collection(db, USERS_COLLECTION);
  return onSnapshot(
    colRef,
    (snapshot) => {
      if (!snapshot.empty) {
        const users: StoredUser[] = snapshot.docs.map((d) => d.data() as StoredUser);
        onUpdate(users);
      } else {
        ensureInitialSeed();
      }
...
/**
 * Save or update user account in Firestore.
 */
export async function saveUserToFirestore(user: StoredUser): Promise<void> {
  try {
...
/**
 * Reset all league data in Firestore to the official initial state for the new season.
 */
...
export async function saveSystemLogToFirestore(log: SystemLog): Promise<void> {
  try {
    ...
    // Do not rethrow so logging never crashes callers
  }
}
...
export async function saveNotificationToFirestore(notification: LeagueNotification): Promise<void> {
  try {
    ...
    // Don't throw so notification failures don't block user flows
  }
}
...
export async function markAllNotificationsAsReadInFirestore(
  ...
  try {
    ...
    for (const notifId of notificationIds) {
      ...
      // We will perform a merge set with readBy array containing readerId
      // Alternatively, we can retrieve docs or set directly
