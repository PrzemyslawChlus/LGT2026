...
export const AdminAccountsManager: React.FC<AdminAccountsManagerProps> = ({
  ...
  const handleSendResetLink = async () => {
    ...
    try {
      await resetPasswordWithFirebase(passwordModalUser.email);
      setResetSuccessMsg(`Wysłano oficjalny e-mail Firebase z linkiem do resetowania hasła na adres: ${passwordModalUser.email}`);
      showFeedback('success', `Link do resetu hasła został wysłany na adres ${passwordModalUser.email}.`);
    } catch (err: unknown) {
      showFeedback('error', err instanceof Error ? err.message : 'Błąd podczas wysyłania linku resetującego.');
    } finally {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await deleteStoredUser(deleteModalUser.id);
      onRefreshUsers();
      showFeedback(
        'success',
        `Konto gracza ${deleteModalUser.name} zostało usunięte z bazy Firestore.`
      );
      setDeleteModalUser(null);
    } catch (err: unknown) {
  ...
  return (
    <div className="bg-white rounded-3xl p-5 sm:p-7 shadow-sm border-2 border-amber-400/80 relative overflow-hidden">
              ...
              >
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 shrink-0 pt-2 lg:pt-0 border-t lg:border-t-0 border-stone-200">
                    {/* 1. Send Password Reset Email Button */}
                    <button
                      type="button"
                      ...
                      title="Wyślij link do resetu hasła Firebase Auth"
                    >
      ...
      {/* MODAL: RESET PASSWORD VIA FIREBASE AUTH */}
      {passwordModalUser && (
          ...
          <div className="bg-white rounded-3xl shadow-2xl border border-stone-200 w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-5 bg-gradient-to-r from-emerald-950 to-stone-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-sm sm:text-base">Reset hasła gracza</h3>
                  <p className="text-[11px] text-stone-300">
                    Wysyłka bezpiecznego linku Firebase Auth
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-5 space-y-4 text-xs">
              ...
              <div className="p-3 bg-stone-50 rounded-2xl border border-stone-200 space-y-1">
                ...
                <div className="font-mono text-stone-600">
                  Adres e-mail: {passwordModalUser.email}
                </div>
              </div>

              <p className="text-stone-600 leading-relaxed">
                Zgodnie z polityką bezpieczeństwa, hasła użytkowników nie są przechowywane w bazie danych ligi.
                Kliknij poniższy przycisk, aby wysłać na adres gracza oficjalną wiadomość z Firebase z jednorazowym linkiem do ustawienia nowego hasła.
              </p>
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-stone-200">
                ...
                >
                  {resetSuccessMsg ? 'Zamknij' : 'Anuluj'}
                </button>
                <button
                  type="button"
                  disabled={isSendingReset}
                  onClick={handleSendResetLink}
                  className="px-5 py-2 rounded-xl font-bold text-white bg-emerald-800 hover:bg-emerald-900 shadow-sm transition-all cursor-pointer flex items-center gap-1.5 disabled:opacity-75"
                >
                  {isSendingReset ? (
                    <>
