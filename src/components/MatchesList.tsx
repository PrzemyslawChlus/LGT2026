...
export const MatchesList: React.FC<MatchesListProps> = ({
  ...
  const renderScheduledCard = (match: Match) => {
    ...
    const isOverdue = isScheduledMatchOverdue(match, 0);

    return (
      <div
        key={match.id}
        className={`bg-white rounded-2xl shadow-xs border p-3.5 sm:p-4 flex flex-col justify-between w-full min-w-0 overflow-hidden ${
          isOverdue
            ? 'border-orange-400 bg-gradient-to-b from-white via-orange-50/20 to-orange-50/40 ring-1 ring-orange-300'
            : 'border-amber-300/80 bg-gradient-to-b from-white to-amber-50/20'
        }`}
      >
        <div className="w-full min-w-0">
          ...
          <div className="pb-3 border-b border-stone-100 space-y-2 min-w-0">
            <div className="flex items-center justify-between gap-2 min-w-0">
              ...
              {isOverdue ? (
                <span className="text-[11px] sm:text-xs font-bold px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full bg-orange-100 text-orange-950 border border-orange-300 whitespace-nowrap animate-pulse flex items-center gap-1">
          ...
          {/* Overdue Match Prompt Banner */}
          {isOverdue && (
            <div className="bg-orange-100/90 border border-orange-300 rounded-xl p-2.5 mb-3 flex items-center justify-between gap-2 text-xs">
              <div className="flex items-center gap-1.5 text-orange-950 font-bold min-w-0">
                <Clock className="w-3.5 h-3.5 text-orange-700 shrink-0" />
                <span className="truncate">Termin minął – czy mecz się odbył?</span>
              </div>
              <button
                type="button"
                onClick={() => setMatchToDelete(match)}
                className="text-rose-700 hover:text-rose-900 font-black hover:underline shrink-0 text-[11px] cursor-pointer"
                title="Usuń wpis o zaplanowanym meczu, jeśli się nie odbył"
              >
                Nie odbył się (usuń)
              </button>
            </div>
