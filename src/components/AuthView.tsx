...
import { authenticate, registerUser, RegistrationInput, getStoredUsers } from '../utils/auth';
...
export const AuthView: React.FC<AuthViewProps> = ({
  ...
  const handleRegPhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Strip leading +48 or 48 if user pastes or types it
    val = val.replace(/^\+?48\s*/, '');
    // Only allow digits up to 9 numbers
    const digits = val.replace(/\D/g, '').slice(0, 9);
    // Format cleanly as XXX XXX XXX
    let formatted = '';
  ...
  // State when registration is submitted and awaiting admin approval
  const [registeredPendingUser, setRegisteredPendingUser] = useState<StoredUser | null>(null);
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    try {
      const user = authenticate(loginEmail, loginPassword);
      onLoginSuccess(user);
    } catch (err: unknown) {
      if (err instanceof Error) {
        setLoginError(err.message);
      } else {
        setLoginError('Wystąpił błąd logowania.');
      }
    }
  };
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (!regName.trim() || !regEmail.trim() || !regPhone.trim() || !regPassword.trim()) {
      ...
      logEvent({
        ...
        level: 'warn',
        userName: regName.trim() || undefined,
        userEmail: regEmail.trim() || undefined,
        details: `Formularz rejestracji odrzucony: brak wymaganych pól (imię: "${regName}", email: "${regEmail}", tel: "${regPhone}").`,
      }).catch(() => {});
      return;
    }
    ...
    if (cleanPhoneDigits.length < 9) {
      ...
      logEvent({
        ...
        details: `Formularz rejestracji odrzucony: niepoprawny format numeru telefonu ("${regPhone}").`,
      }).catch(() => {});
      return;
    }
    ...
    if (regPassword.length < 6) {
      ...
      logEvent({
        ...
        userName: regName.trim(),
        userEmail: regEmail.trim(),
        details: `Formularz rejestracji odrzucony: hasło zbyt krótkie (< 6 znaków).`,
      }).catch(() => {});
      return;
    }
    ...
    if (regPassword !== regConfirmPassword) {
      ...
      logEvent({
        ...
        userName: regName.trim(),
        userEmail: regEmail.trim(),
        details: `Formularz rejestracji odrzucony: hasła nie są identyczne.`,
      }).catch(() => {});
      return;
    }
    ...
    if (!fairPlayPledge) {
      ...
      logEvent({
        ...
        userName: regName.trim(),
        userEmail: regEmail.trim(),
        details: `Formularz rejestracji odrzucony: brak zaznaczenia akceptacji Kodeksu Fair Play.`,
      }).catch(() => {});
      return;
    }
  ...
  return (
    <div className="min-h-screen w-full max-w-full bg-stone-950 flex flex-col justify-center items-center px-4 py-8 sm:py-12 relative overflow-x-hidden overflow-y-auto font-sans selection:bg-amber-400 selection:text-emerald-950">
      {/* Background Decorative Atmosphere (strictly contained to prevent horizontal scroll) */}
      ...
      <div className="relative z-10 w-full max-w-md min-w-0">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-xs sm:text-sm text-emerald-300/90 mt-1 font-medium flex items-center justify-center gap-1.5">
            ...
            <span>Strefa Członkowska • Dostęp autoryzowany</span>
          </p>
        </div>
            ...
            <div className="p-6 sm:p-7 text-center space-y-4 w-full min-w-0">
              ...
              <div className="w-full min-w-0">
                ...
                <p className="text-xs text-stone-600 mt-2 leading-relaxed break-words">
                  ...
                  )}! Twoje konto zostało utworzone w rejestrze ligi.
                </p>
              </div>
              ...
              <div className="p-3.5 bg-amber-50/90 border border-amber-200/90 rounded-2xl text-left space-y-1.5 w-full min-w-0">
                <p className="font-bold text-xs flex items-center gap-1.5 text-amber-950">
                  ...
                  <span>Wymagane potwierdzenie administratora</span>
                </p>
                <p className="text-[11px] text-stone-700 leading-normal break-words">
                  W celu ochrony ligi przed niechcianym ruchem z zewnątrz, <strong>Komisarz Ligi (Admin)</strong> musi zatwierdzić Twoje konto w sekcji <em>„Zasady i opcje”</em>.
                </p>
                <p className="text-[11px] text-stone-700 leading-normal font-medium break-words">
                  Po zatwierdzeniu profilu automatycznie staniesz się oficjalnym graczem ligi i uzyskasz pełny dostęp do logowania.
                </p>
              </div>
            ...
            {activeMode === 'login' ? (
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex items-center justify-end mt-1 text-[10px] text-stone-400">
                    <button
                      type="button"
                      onClick={() => setShowForgotPasswordModal(true)}
                      className="text-stone-500 hover:text-emerald-900 hover:underline cursor-pointer"
                    >
                ...
                <button
                  type="submit"
                  className="w-full py-3 px-4 bg-gradient-to-r from-emerald-900 via-emerald-800 to-emerald-950 hover:from-emerald-800 hover:to-emerald-900 text-white font-bold rounded-2xl text-sm shadow-md transition-all cursor-pointer flex items-center justify-center gap-2 border border-emerald-700/50"
                >
              ...
              <form onSubmit={handleRegisterSubmit} className="space-y-3.5">
                ...
                {regNickname.trim() && (
                  <div className="px-3 py-1.5 bg-emerald-50 border border-emerald-200 rounded-xl text-[11px] text-emerald-900 flex items-center gap-1.5 animate-in fade-in">
                    <span className="font-semibold">Wizualizacja na karcie gracza:</span>
                    <span>{regName.trim() || 'Zawodnik'}</span>
                    <span className="font-bold text-emerald-800 italic">„{regNickname.trim()}”</span>
                  </div>
                )}
                ...
                >
                  {isSubmittingReg ? (
                    <>
                      ...
                      <span>Rejestrowanie dżentelmena...</span>
                    </>
                  ) : (
                    <>
        ...
        <div className="mt-4 text-center text-[11px] text-emerald-300/60 flex items-center justify-center gap-1.5">
          ...
          <span>Aplikacja zabezpieczona • Baza ligi chroniona sesją</span>
        </div>
      </div>
          ...
          >
            ...
            <div className="p-5 bg-gradient-to-r from-emerald-950 via-emerald-900 to-stone-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-sm sm:text-base">Zapomniałeś hasła?</h3>
                  <p className="text-[11px] text-stone-300">Liga Tenisowa Gentlemanów</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="p-6 space-y-4">
              <div className="w-14 h-14 mx-auto rounded-2xl bg-amber-50 border border-amber-200 flex items-center justify-center shadow-xs">
                <KeyRound className="w-7 h-7 text-amber-600" />
              </div>

              <div className="text-center space-y-2">
                <p className="text-sm sm:text-base text-stone-800 leading-relaxed font-semibold bg-stone-50 p-4 rounded-2xl border border-stone-200 shadow-xs">
                  Jeśli zapomniałeś hasła skontaktuj się z <strong>Przemkiem</strong> lub <strong>Ładakiem</strong>. Ustawią Ci nowe.
                </p>
                <p className="text-xs text-stone-500 leading-normal">
                  Administratorzy ligi posiadają bezpośredni dostęp do generatora haseł i mogą natychmiast nadać Ci nowy kod dostępu.
                </p>
              </div>
